...
import cn.yhjz.camera.cache.EventConstant;
...
public class FMSGCallBack_V31 implements HCNetSDK.FMSGCallBack_V31 {
    ...
    public FMSGCallBack_V31 (Camera camera) {
        this.camera = camera;
    }
        ...
        switch (lCommand) {
            ...
            case HCNetSDK.COMM_ALARM_RULE:
                ...
                switch (strVcaAlarm.struRuleInfo.wEventTypeEx) {
                    case 1: //穿越警戒面 (越界侦测)
                        saveImgToBase64(EventConstant.actionInfo.get(1));
                        System.out.println("穿越警戒面报警发生");
                        break;
                    case 2: //目标进入区域
                        saveImgToBase64(EventConstant.actionInfo.get(2));
                        System.out.println("目标进入区域报警发生");
                        break;
                    case 3: //目标离开区域
                        saveImgToBase64(EventConstant.actionInfo.get(3));
                        System.out.println("目标离开区域报警触发");
                    case 4: //周界入侵
                        saveImgToBase64(EventConstant.actionInfo.get(4));
                        System.out.println("周界入侵报警发生");
                        break;
                    case 5: //徘徊
                        saveImgToBase64(EventConstant.actionInfo.get(5));
                        System.out.println("徘徊事件触发");
                        break;
                    case 8: //快速移动(奔跑)，
                        saveImgToBase64(EventConstant.actionInfo.get(8));
                        System.out.println("快速移动(奔跑)事件触发");
                        break;
                    case 15: //离岗
                        saveImgToBase64(EventConstant.actionInfo.get(15));
                        System.out.println("离岗事件触发");
                    case 20: //倒地检测
                        saveImgToBase64(EventConstant.actionInfo.get(20));
...
//                    String nowtime = sf.format(new Date());
            ...
            case HCNetSDK.COMM_ALARM_V30:  //移动侦测、视频丢失、遮挡、IO信号量等报警信息(V3.0以上版本支持的设备)
                ...
                struAlarmInfo.read();
                System.out.println("报警类型：" + EventConstant.dwAlarmType.get(struAlarmInfo.dwAlarmType));  // 3-移动侦测
                saveImgToBase64(EventConstant.dwAlarmType.get(struAlarmInfo.dwAlarmType));
                break;
     ...
     * 保存图片转base64
     */
    public void saveImgToBase64(String eventType) {

        List<AbstractCamera> aimCameraList = CameraHolder.cameraList.stream().filter(
                (c) -> c.getId().equals(camera.getId())
        ...
        eventJson.put("eventJson", eventType);
        eventJson.put("timestamp", new Date());

        JSONObject bodyJson = new JSONObject();
        bodyJson.put("cameraId", camera.getId());
        bodyJson.put("imgBase64", aimCamera.capture());
